...
interface IUseEvent {
  getEvents: () => Promise<SockbaseEventDocument[]>
...
const useEvent = (): IUseEvent => {
  ...
  const getEvents = async (): Promise<SockbaseEventDocument[]> => {
    const db = getFirestore()
    const eventRef = FirestoreDB
      .collection(db, '/events')
      .withConverter(eventConverter)
    const eventQuery = FirestoreDB.query(eventRef)
    const eventSnapshots = await FirestoreDB.getDocs(eventQuery)
