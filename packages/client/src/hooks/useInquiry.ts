...
export const inquiryMetaConverter: FirestoreDB.FirestoreDataConverter<SockbaseInquiryMetaDocument> = {
  ...
  fromFirestore: (snapshot: FirestoreDB.QueryDocumentSnapshot): SockbaseInquiryMetaDocument => {
...
const useInquiry: () => IUseInquiry =
  () => {
    ...
    const getInquiries: () => Promise<SockbaseInquiryDocument[]> =
      ...
      const getInquiryByIdAsync = async (inquiryId: string): Promise<SockbaseInquiryDocument> => {
        const db = getFirestore()
      ...
      const getInquiryMetaByInquiryIdAsync = async (inquiryId: string): Promise<SockbaseInquiryMetaDocument> => {
        const db = getFirestore()
      ...
      const getInquiryType = (inquiryType: string): { type: string; name: string; description: string; } => 
        inquiryHelper.inquiryTypes.filter(t => t.type === inquiryType)[0]
      
      const setStatusByIdAsync = async (inquiryId: string, status: SockbaseInquiryStatus): Promise<void> => {
        const db = getFirestore()
        const inquiryMetaRef = FirestoreDB.doc(db, `/_inquiries/${inquiryId}/private/meta`)
          .withConverter(inquiryMetaConverter)

        await FirestoreDB.setDoc(inquiryMetaRef, {
          status
        }, {
          merge: true
        })
